...
`get_recorddataadditional` allows to use additional arguments such as `dataTypeGroupIds` and `dataTypeGroupId2s`.For a list of all arguments you can use, see [here](https://population.un.org/demodata-api/swagger/index.html).  For example:
...
Currently the IP of the server is automatically set (to `https://population.un.org/demodata-api/api/`) but can be changed simply by adding `options(unpd_server = "your server")` to the beginning of your script:
...
Figuring out which specific set of values returns what the user want is a tedious process. If the user doesn't know the specific combination of values by memory, the most optimal strategy that we advise is to explore the available values in the [API documentation](https://population.un.org/demodata-api/swagger/index.html) or, from within R, with the reference helpers such as `get_locationtypes` and `get_subgroups`, and then use these in the request.

For example, `get_locationtypes` and `get_subgroups` (restricted to the same indicator and country) list which `locAreaTypeIds` and `subGroupIds` are available for a given combination. In the example above, `"Semi-rural"` is not among the location types returned for register data on deaths by age and sex in `Oman`, which is why the request errors.

Using these helpers to inspect the available values is much easier and less time consuming than trying out different values.
